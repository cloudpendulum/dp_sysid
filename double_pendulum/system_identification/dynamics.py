...
class yb_matrix_sym():
    def __init__(self,
        ...
        # b1, b2, tau_c1, tau_c2 = smp.symbols("b1 b2 tau_c1 tau_c2")
        # friction = smp.Matrix([
        #     b1 * plant.qd1 + tau_c1 * smp.sign(plant.qd1),
        #     b2 * plant.qd2 + tau_c2 * smp.sign(plant.qd2)
        # ])
        # eom = plant.M*plant.qdd + plant.C*plant.qd - plant.G + friction
    ...
    def __call__(self, q_vec, dq_vec, ddq_vec):
        ...
        #qqq = list(q_vec) + list(dq_vec) + list(ddq_vec)
        yb = self.yb_l(*qqq)
